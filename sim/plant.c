...
// ============================================================================
// Barometric Altitude Mock Physics Generator (7.11 complementary filter ground truth)
// ============================================================================

void plant_generate_baro(float *out_pressure_pa, float *out_temp_c, uint32_t ts_us) {
if (!out_pressure_pa || !out_temp_c) return;

uint32_t local_seed = global_seed + ts_us + 44;

// Standard Sea Level Parameters
const float P0 = 101325.0f;       // Sea Level Pressure (Pa)
const float T0_k = 288.15f;       // Sea Level Temp (15 degC in Kelvin)
const float lapse_rate = 0.0065f; // K/m

// Drone's ground-truth vertical position
float z_world = global_state.position.z;

// Standard Atmosphere Temperature lapse logic
float temp_k = T0_k - lapse_rate * z_world;
if (temp_k < 200.0f) temp_k = 200.0f; // Physical lower limit clamp

// Standard Hypsometric Equation for Pressure: P = P0 * (T / T0) ^ (g * M / R * L)
// where g*M / (R*L) is approximately 5.25588f
float pressure_pa = P0 * powf((temp_k / T0_k), 5.25588f);

// Apply high-fidelity sensor noise (simulating MS5611 characteristics)
// Real-world MS5611 has roughly ~1.5 to 2.5 Pascals variance
float noise_pressure = generate_gaussian(&local_seed, 2.0f);   // 2.0 Pa variance
float noise_temp = generate_gaussian(&local_seed, 0.05f);      // 0.05 degC variance

*out_pressure_pa = pressure_pa + noise_pressure;
*out_temp_c = (temp_k - 273.15f) + noise_temp;
